...
export function AdvancedTrendChart({
    ...
    if (allDates.length > 0) {
        baseStartDate = new Date(allDates[0] + 'T12:00:00')
    }
    baseStartDate.setHours(12, 0, 0, 0)
    
    let baseEndDate = new Date()
    baseEndDate.setHours(12, 0, 0, 0)
    ...
    for (const f of flights) {
        ...
        const imSafe = assessments.find(a => a.id === f.imSafeAssessmentId && 'illnessLevel' in a)
        const pave = assessments.find(a => a.id === f.paveAssessmentId && 'pilotRiskLevel' in a)
    ...
    const visibleStartIdx = Math.max(0, days.length - (currentOffset + 1) * DAYS_TO_SHOW)

// src/pages/offline/OfflineDashboard.tsx
import { useEffect, useState } from 'react'
import { useModeStore } from '../../stores/modeStore'
import { offlineDashboardApi, offlineImSafeApi, offlinePaveApi, offlineFlightApi, offlineMaintenanceApi } from '../../lib/offlineBridge'
...
import { Users, TrendingUp, XCircle, RefreshCw, BarChart3, Plane, Activity, AlertTriangle } from 'lucide-react'
...
export function OfflineDashboard() {
    ...
    const load = async () => {
        ...
        setError(null)
        try {
            const [statsData, imData, pvData, flData] = await Promise.all([
                offlineDashboardApi.getStats(),
                offlineImSafeApi.getAll(),
                offlinePaveApi.getAll(),
                offlineFlightApi.getAll(),
            ])
            setStats(statsData)
            const combined = [...imData, ...pvData]
            ...
            setError('Could not load local data. Ensure you are running the desktop app.')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleResetAndReseed = async () => {
        setReseeding(true)
        try {
            await offlineMaintenanceApi.resetAndReseed()
            await load()
        } finally {
            setReseeding(false)
        }
    ...
    return (
        <div className="space-y-4 sm:space-y-6 w-full overflow-hidden">
            <div className="glass-card p-4 sm:p-5 border-2 border-blue-200 bg-blue-50/70">
                <div className="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
                    <div>
                        <h2 className="text-lg font-black text-slate-900">Seed Data Tools</h2>
                        <p className="text-sm font-medium text-slate-700 mt-1">
                            If you already had old offline data, press this once to reset local store and reseed 15 demo flights for current-week charts.
                        </p>
                    </div>
                    <button
                        onClick={handleResetAndReseed}
                        disabled={reseeding}
                        className="px-5 py-3 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-bold disabled:opacity-60"
                    >
                        {reseeding ? 'Resetting...' : 'Reset + Reseed Demo Data'}
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <MetricCard value={stats.totalPilots || 1} label="Total Pilots" icon={<Users size={18} className="text-primary-500" />} />
                <MetricCard value={stats.activePilots || 1} label="Active Pilots" icon={<Activity size={18} className="text-green-600" />} color="text-green-600" />
